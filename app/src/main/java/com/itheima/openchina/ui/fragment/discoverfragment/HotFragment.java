...
public class HotFragment extends BaseFragment {
    ...
    protected View onCreateContentView() {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                String url = hotBean.typeBean.list.get(position).url;
    ...
    public class Myadapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            //给控件设置数据
            viewHolder.viewcontent.setText(hotBean.typeBean.list.get(position).name);
            viewHolder.viewdesc.setText(hotBean.typeBean.list.get(position).description);
            return convertView;
        }
        //设置ViewHolder
    ...
    protected void onStartLoadData() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url = "http://www.oschina.net/action/api/software_list?pageIndex=0&searchTag=view&pageSize=20";
                String stringData = LoadData.getInstance().getStringData(url);
                hotBean = XmlUtils.toBean(RecommondBean.class, stringData.getBytes());

                size = hotBean.typeBean.list.size();
